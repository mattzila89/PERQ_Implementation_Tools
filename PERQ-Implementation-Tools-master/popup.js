...
resizeWindow.onclick = function () {
    chrome.tabs.reload();
}

hubExp.onclick = function () {
    chrome.tabs.reload();
}
...
showCIJiraColors.onclick = function () {
    chrome.tabs.query({'active': true, 'lastFocusedWindow': true}, function (tabs) {
        var url = tabs[0].url;
        if (url.indexOf("https://perqindianapolis.atlassian.net/projects/CI/queues/custom/")>-1) {
            chrome.tabs.reload();
        }
    });
}
...
showPTSDJiraColors.onclick = function () {
    chrome.tabs.query({'active': true, 'lastFocusedWindow': true}, function (tabs) {
        var url = tabs[0].url;
        if (url.indexOf("https://perqindianapolis.atlassian.net/projects/PTSD/queues/custom/")>-1) {
            chrome.tabs.reload();
        }
    });
}
