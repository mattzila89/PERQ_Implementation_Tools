...
const injectHTML = () => {
    ...
    if (!localStorage.posTop && !localStorage.posLeft) {
        // Do nothing
    } else {
        // Update location before appending
        modalSection.setAttribute("style", 'top: ' + localStorage.posTop + 'px; left: ' + localStorage.posLeft + 'px;');
    }
...
var hubExp = () => {
    setTimeout(function(){
        let element = document.querySelector("script[src*='.fatwin.com/api/websiteplugins/config/hub']");
        if (element) {
            injectCss('hubExp1');
            injectHTML();
            ...
            function dragElement(elmnt) {
              ...
              function elementDrag(e) {
                ...
                localStorage.posTop = elmnt.offsetTop - pos2;
                localStorage.posLeft = elmnt.offsetLeft - pos1;
              }
            ...
            document.getElementById("selectorTool").addEventListener("click", function(e){
                ...
                javascript:(function(){
                    let sGadget=document.createElement('script');
                ...
                setTimeout(function(){
                    ...
                    let copyText = document.getElementById("_sg_path_field");
                    copyText.select();
            ...
            // Do nothing
            console.log("Hub doesn't exist - Please turn on Fatwin extension or try the testing tool on a live site");
        }
    }, 1000);
};
    ...
    const prioritize = () => {
      ...
      list.forEach(item => {
        if (item) {
          ...
          let dueTimeText = item.querySelector('sc-btewqU').textContent.trim();
          let tempVal = dueTimeText.split(" ");
